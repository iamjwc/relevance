...
module Relevance
  ...
  def _nt_stmt
    ...
    i0 = index
    i1, s1 = index, []
    r2 = _nt_comparison_statment
    s1 << r2
    if r2
      r3 = _nt_combination_operator
      s1 << r3
      if r3
        r4 = _nt_stmt
        s1 << r4
  ...
  module ComparisonStatment1
    def tree
      [i.tree, op.tree, i2.tree]
    end
  end

  def _nt_comparison_statment
    start_index = index
    if node_cache[:comparison_statment].has_key?(index)
      cached = node_cache[:comparison_statment][index]
      if cached
        cached = SyntaxNode.new(input, index...(index + 1)) if cached == true
        @index = cached.interval.end
      end
    ...
    if s1.last
      r1 = instantiate_node(SyntaxNode,input, i1...index, s1)
      r1.extend(ComparisonStatment0)
      r1.extend(ComparisonStatment1)
    else
      @index = i1
      r1 = nil
    end
    ...
    node_cache[:comparison_statment][start_index] = r0
  ...
  module CombinationOperator1
    def tree
      op.text_value.to_sym
    end
  end

  def _nt_combination_operator
    start_index = index
    if node_cache[:combination_operator].has_key?(index)
      cached = node_cache[:combination_operator][index]
      if cached
        cached = SyntaxNode.new(input, index...(index + 1)) if cached == true
        @index = cached.interval.end
      end
    ...
    if s0.last
      r0 = instantiate_node(SyntaxNode,input, i0...index, s0)
      r0.extend(CombinationOperator0)
      r0.extend(CombinationOperator1)
    else
      @index = i0
      r0 = nil
    end

    node_cache[:combination_operator][start_index] = r0
  ...
  def _nt_combination_op
    start_index = index
    if node_cache[:combination_op].has_key?(index)
      cached = node_cache[:combination_op][index]
      if cached
        cached = SyntaxNode.new(input, index...(index + 1)) if cached == true
        @index = cached.interval.end
      end
    ...
    node_cache[:combination_op][start_index] = r0
